...
The arduino code has been designed to extract the necessary information from the model_properties_CSCMult.h file
This class` purpose is to take a model and fill the file adquatly 

This is part of the series convertToC, convertToCSC. Here we employ a CSC format, which has been 
developed to take less time to executing taking advantage of sorting weights so that
less multiplications have to be done
...
If "bit" there is bit compression happening for the labels
...
class convertToCCSC():
    ...
    def __init__(self,model,intOrByte):
        self.model = model
        if intOrByte == "int":
            self.fileNameProperties = "convertOutput/model_properties_CSCMult.h"
        elif intOrByte == "byte":
            self.fileNameProperties = "convertOutput/model_properties_CSCMultByte.h"
        else:
            self.fileNameProperties = "convertOutput/model_properties_CSCMultBit.h"
    ...
    def extractDataFromModel(self):
        ...
        self.labelptrPerLayer = np.zeros((self.numberOfLayers),dtype = object)
        ...
        for layerCounter in range(len(self.model.layers)):
            ...
            centroids,labels,labelptr,indices,indptr = mycsc.compressMatrixSaveMultiplication()

            self.centroidsPerLayer[layerCounter] = centroids
            # compress labels to 4 bit
            if self.intOrByte == "byte" or self.intOrByte == "int":
                self.labelsPerLayer[layerCounter] = labels
            elif self.intOrByte == "bit":
                # compress labels to fewer bits
                self.labelsPerLayer[layerCounter] = self.compressToFewBits(layerCounter,labels)

            self.labelptrPerLayer[layerCounter] = labelptr
            self.indicesPerLayer[layerCounter] = indices
            self.numberOfNonZeroValuesPerLayer[layerCounter] = len(indices)
    ...
    def calculateTheoreticalSize(self):
        ...
        for i in self.indicesPerLayer:
            if self.intOrByte == "int":
                numberOfInts += len(i)
            elif self.intOrByte == "byte" or self.intOrByte == "bit":
                numberOfBytes+= len(i)                    
        for i in self.labelsPerLayer:
            if self.intOrByte == "int":
                ...
                numberOfBytes+= len(i)                        
        for i in self.labelptrPerLayer:
            if self.intOrByte == "int":
                numberOfInts += len(i)
            elif self.intOrByte == "byte" or self.intOrByte == "bit":
                numberOfBytes+= len(i)                     
        ...
        for i in self.indptrPerLayer:
            numberOfInts+=len(i)
        for _ in self.numberOfNonZeroValuesPerLayer:
            numberOfInts+=1
    ...
    def defineStructs(self):
        ...
        if self.intOrByte == "int":
            ...
            writer.writerow(['  int* labelptr;'])        
            writer.writerow(['  int* indices;'])
        elif self.intOrByte == "byte" or self.intOrByte=="bit":
            writer.writerow(['  unsigned char* labels;'])
            writer.writerow(['  unsigned char* labelptr;'])        
    ...
    def writeContent(self):
        ...
        for layerCounter in range(self.numberOfLayers):
            ...
            if self.intOrByte == "int":
                ...
                writer.writerow(["const int labelptrLayer"+str(layerCounter+1)+" [] PROGMEM = {"+self.unpackArray(self.labelptrPerLayer[layerCounter])+"};"])
                writer.writerow(["const int indicesLayer"+str(layerCounter+1)+" [] PROGMEM = {"+self.unpackArray(self.indicesPerLayer[layerCounter])+"};"])
            elif self.intOrByte == "byte":
                ...
                writer.writerow(["const unsigned char labelptrLayer"+str(layerCounter+1)+" [] PROGMEM = {"+self.unpackArray(self.labelptrPerLayer[layerCounter])+"};"])
    ...
    def writeStructs(self):
        ...
        for layerCounter in range(self.numberOfLayers):
            ...
            writer.writerow(["  .labelptr = labelptrLayer"+str(layerCounter+1)+","])
    ...
    """
    compressToFewBits
    The final model was quantized to 16 and 4 weights in the respective layers.
    We know want to put more than 1 value in 8 bit 
    """
    def compressToFewBits(self,layercounter,array):
        # In first layer one value takes 4 bit of the 8 and in second only 2
        if layercounter == 0:
            valuesPerByte = 4
        else:
            valuesPerByte = 2
        newarray = np.array([]) # where the compressed values will be stored
        i = 0   # iterate over the values in the original array
        done = False
        while not done: # stop only when we reached end of original array

            compressedValue = 0 # this is where the values get "compressed","pushed" into
            
            # here we push the values in the compressedValue, We can do that 2 times for the first and 4 times for the second layer
            for counter in range(int(8/valuesPerByte),0,-1):
                # read value from array
                oldval = int(array[i]) 
                # shift old value one time less to the left than the value before
                for a in range(counter-1):
                    oldval = oldval<<valuesPerByte
                
                # add the shifted value to the compressed value
                compressedValue += oldval

                # increment original array
                i = i+1
                # check if we reached end of array, break!
                if i == len(array):
                    done = True
                    break
            # push compressed value on top of the new array
            newarray = np.append(newarray,compressedValue)
        return newarray
