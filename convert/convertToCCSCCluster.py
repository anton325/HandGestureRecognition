"""
convertToCCSCCluster
This modul can translate the with keras generated and trained model to the C code
The arduino code has been designed to extract the necessary information from the model_properties_CSCCluster.h file
This class` purpose is to take a model and fill the file adquatly 


It writes two files, one the model_propertiesCSCCluster.h file with the weights and the 
...
class convertToCCSC():
    ...
    def __init__(self,model,intOrByte):
        self.model = model
        if intOrByte == "int":
            self.fileNameProperties = "convertOutput/model_properties_CSCCluster.h"
        else:
            self.fileNameProperties = "convertOutput/model_properties_CSCClusterByte.h"
    ...
    def extractDataFromModel(self):
        ...
        for layerCounter in range(len(self.model.layers)):
            ...
            quantizedData = mycsc.quantizeData(data)
    ...
    def calculateTheoreticalSize(self):
        ...
        numberOfInts = 0
        numberOfBytes = 0
        for d in self.centroidsPerLayer:
