...
void Buffer::_resetBuffer() {
  ...
  _tresholdLow = _ema * (1 - _margin);
  _tresholdHi = _ema * (1 + _margin);
  _countdownSinceGesture = 0;
  _endBuffer = 0;
}
...
int Buffer::feedFrame(double* frame) {
  ...
  else if (_meanOfThisFrame >= _tresholdLow && _meanOfThisFrame <= _tresholdHi) {
  ...
  else if ((_meanOfThisFrame < _tresholdLow || _meanOfThisFrame > _tresholdHi) && _gestureflag == 0) {
...
double* Buffer::getScaledBuffer() {
  ...
  // let pseudoindexes point to end of buffer
  double* startOfPseudoIndexes = _buffer[_endBuffer];
  ...
  // calculate the interpolated frames AND put the first 5 of them behind the pseudoindexes
  ...
  int framesThatGoBehindBuffer = 6;
