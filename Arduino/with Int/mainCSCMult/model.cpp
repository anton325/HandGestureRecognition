...
#include "model.h"
#include "model_properties_CSCMult.h"
...
double* activationOfOutputlayer(struct model* myModel, double* input) {
  ...
  for (int layerCounter = 0; layerCounter < myModel->layers; layerCounter++) {
    ...
    for(int index = 0 ; index < myModel->layers[layerCounter].numberOfNonZeroValues; index++){
      if(labelsAddedInARow >= pgm_read_word(&myModel->layers[layerCounter].labelptr[labelCount])){
        // apply multiplication
        labelIndex = int(pgm_read_word(&myModel->layers[layerCounter].labels[labelCount]));
      ...
      row = int(pgm_read_word(&myModel->layers[layerCounter].indices[index]));
      sumInputs += input[row];
      labelsAddedInARow++;     
    }
