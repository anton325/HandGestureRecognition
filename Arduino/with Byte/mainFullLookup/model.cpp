...
#include "model.h"
#include "model_properties_FullByte.h"
  ...
  for (int layerCounter = 0; layerCounter < myModel->numInputs; layerCounter++) {
    ...
    for (int inputCounter = 0; inputCounter < numInputs; inputCounter++) {
      for (int weightCounterLeavingThisInput = 0; weightCounterLeavingThisInput < myModel->layers[layerCounter].numNeurons; weightCounterLeavingThisInput++) {
        ...
        thisLabel = pgm_read_byte(&myModel->layers[layerCounter].labels[inputCounter * myModel->layers[layerCounter].numNeurons + weightCounterLeavingThisInput]);
        if(thisLabel != 0){
        weight = pgm_read_float(&myModel->layers[layerCounter].centroids[thisLabel]);
